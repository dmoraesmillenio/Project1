...
$requestTable = 'public.actor_info';
$strTableName = 'public.actor_info';
$requestPage = "list";
...
{	
	...
	$options["mainRecordData"] = runner_json_decode( postvalue('data') );
	$options["mainRecordMasterTable"] = postvalue('mainRecordMasterTable');
	
	if( postvalue("parentsExist") )
		$options["parentCtrlsData"] = runner_json_decode( postvalue("parentCtrlsData") );
}
...
if(	postvalue("mapRefresh") )
{
	...
	$options["vpCoordinates"] = runner_json_decode( postvalue("vpCoordinates") );
}
...
if( ProjectSettings::ext() == 'aspx' && $mode != LIST_SIMPLE ) {
	exit();
}
?>