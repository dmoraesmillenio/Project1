 ...
 */
function runner_http_request( $url, $body = "", $method = "GET", $headers = array(), $certPath = false, $timeout = 60 )
{
	$options = array(
        CURLOPT_URL => $url,
        CURLOPT_RETURNTRANSFER => true,
...
{
	$mailProviderType = ProjectSettings::getSecurityValue( 'emailSettings', 'provider' );
	if( $mailProviderType == empGOOGLE ) {
		require_once( getabspath( 'classes/email/gmail.php' ) );
		$provider = new GmailProvider();
	}
	if( $mailProviderType == empAZURE ) {
		require_once( getabspath( 'classes/email/azure.php' ) );
		$provider = new AzureMailProvider();
	}
	
	if( $provider ) {
		$ret = $provider->send( $params );
		//	legacy code support
		$ret['success'] = $ret['mailed'];
		return $ret;
	}
...
function hash_rs256( $data, $key ) {
	$success = openssl_sign($data, $signature, $key, 'SHA256' );
	if( !$success ) {
 ...
 */
function callDashboardSnippet( $funcName, &$icon, &$header, &$tab ) {
	global $globalEvents;
	ob_start();
	$globalEvents->$funcName( $header, $icon, $tab );
	$ret = ob_get_contents();
	ob_end_clean();
	return $ret;
}
