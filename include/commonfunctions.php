...
{
	//	protect from loading arbitrary file
...
function getLatLngByAddr($addr)
{
	...
	switch( getMapProvider() ){
		case GOOGLE_MAPS:
			$url = 'https://maps.googleapis.com/maps/api/geocode/json?address='.rawurlencode($addr).'&key=' . $apiKey;
...
function request_protocol() {
	...
	if( $_SERVER['HTTP_X_FORWARDED_PROTOCOL'] ) {
		return $_SERVER['HTTP_X_FORWARDED_PROTOCOL'];
	}
	if( $_SERVER['HTTP_X_URL_SCHEME'] ) {
		return $_SERVER['HTTP_X_URL_SCHEME'];
	}
 ...
 * @return RestConnection
 */
function getGoogleEmailConnection() {
	$connData = array();
	$connData["id"] = empidGOOGLE;
	$connData["authType"] = "oauthserver";
	$connData["oauthType"] = oauthCredSERVICEACC;

	$emailSettings = ProjectSettings::getSecurityValue( 'emailSettings' );
	$config = runner_json_decode( $emailSettings['oauthSettingsJson'] );

	$connData['username'] = $emailSettings['oauthUserEmail'];
	$connData["clientId"] = $config['client_id'];
	$connData['client_email'] = $config['client_email'];
	$connData["private_key"] = $config['private_key'];
	$connData["scope"] = 'https://www.googleapis.com/auth/gmail.send https://mail.google.com';

	$connData["url"] = 'https://www.googleapis.com/gmail/v1/';
	$connData["authUrl"] = $config['auth_uri'];
	$connData["tokenUrl"] = 'https://www.googleapis.com/oauth2/v4/token';

	return new RestConnection( $connData );
}

function getAzureEmailConnection() {
	$connData = array();
	$connData["id"] = empidAZURE;
	$connData["authType"] = "oauthserver";
	$connData["oauthType"] = oauthCredCLIENT;

	$emailSettings = ProjectSettings::getSecurityValue( 'emailSettings' );

	$connData["clientId"] = $emailSettings['oauthClientId'];
	$connData["clientSecret"] = $emailSettings['oauthClientSecret'];
	$connData["scope"] = 'https://graph.microsoft.com/.default';

	//$connData["url"] = 'https://www.googleapis.com/gmail/v1/';
	$connData["tokenUrl"] = 'https://login.microsoftonline.com/' . $emailSettings['oauthTenantId'] . '/oauth2/v2.0/token';
...
function createStorageObject( $providerType, $additionalParams ) {
	...
	if( $providerType == stpGOOGLECLOUD ) {
		$params["accessKey"] = ProjectSettings::googleCloudAccessKey();
		$params["secretKey"] = ProjectSettings::googleCloudSecret();
		$params["bucket"] = ProjectSettings::googleCloudBucket();
		$params = array_merge( $params, $additionalParams );
		return new GoogleCloudFileSystem( $params );
	}
}
...
function getStorageProvider( $pSet, $field ) {
	...
	} else if( $providerType == stpGOOGLECLOUD) {
		$params["path"] = $pSet->googleCloudPath( $field );
	} else if( $providerType == stpAZUREBLOB) {
		$params["path"] = $pSet->amazonPath( $field );
