...
class ProjectSettings
{
	...
	public function pageAvailable( $page ) {
		return array_key_exists( $page, $this->getPageIds() ) !== FALSE;
	}
...
//		global $tables_data, $field_labels, $fieldToolTips, $placeHolders, $page_titles, $detailsTablesData, $masterTablesData, $bSubqueriesSupported;
	...
	*/
	...
	function getLookupType( $field ) {
		...
		$tableName = $this->getLookupTable( $field );
		$tableObj = @$projectTables[ $tableName ];
		if( $tableObj && $tableObj[ 'connId' ] == $this->getNotProjectLookupTableConnId( $field )) {
			return LT_QUERY;
		} else {
			return LT_LOOKUPTABLE;
		}
	}
	...
	function getScrollGridBody()
	{
		...
		return $this->getTableValue( 'scrollGridBody' ) && $this->getListGridLayout() != gltCARDS;
	}
	...
	function googleCloudPath( $field ) {
		return $this->getFieldValue( $field, 'googleCloudPath' );
	}
	function azureBlobPath( $field ) {
		return $this->getFieldValue( $field, 'azureBlobPath' );
	}
	...
	static function googleCloudSecret() {
		return ProjectSettings::getProjectValue( 'cloudSettings', 'cloudGoogleCloudSecret' );
	}

	static function googleCloudAccessKey() {
		return ProjectSettings::getProjectValue( 'cloudSettings', 'cloudGoogleCloudAccessKey' );
	}

	static function googleCloudBucket() {
		return ProjectSettings::getProjectValue( 'cloudSettings', 'cloudGoogleCloudBucket' );
	}
	...
	function showCopyLink( $field ) {
		return $this->getFieldValue( $field, 'view', 'copyClipboard' );
	}

	function viewLinkTable( $field ) {
		return $this->getFieldValue( $field, 'viewLinkTable' );
	}

	function viewLinkLookup( $field ) {
		return $this->getFieldValue( $field, 'viewLinkLookup' );
	}

	function showViewLink( $field ) {
		return $this->getFieldValue( $field, 'view', 'viewLink' );
	}

	function viewLinkMethod( $field ) {
		return $this->getFieldValue( $field, 'view', 'viewLinkMethod' );
	}

	function viewLinkPage( $field ) {
		return $this->getFieldValue( $field, 'view', 'viewLinkPage' );
	}

	function checkboxInLabel( $field ) {
		return $this->getFieldValue( $field, 'edit', 'checkboxInLabel' );
	}

	function checkboxInLabelView( $field ) {
		return $this->getFieldValue( $field, 'view', 'checkboxInLabel' );
	}
