...
class ViewControl
{
	...
	/**
	 * Used in View as Custom together with Lookup wizard
	 */
	public $displayField = null;

	/**
	 * Used in View as Text together with Lookup wizard
	 */
	public $linkFieldValue = null;
	...
	*/
	...
	public function showDBValue(&$data, $keylink, $html = true )
	{
		...
		if( $html ) {
			$processedText = nl2br( $processedText );
			
			if( $this->pageObject && RunnerPage::interactiveHtmlPageType( $this->container->pageType ) ) {
				
				$viewLinkTable = $pSet->viewLinkTable( $this->field );
				if( $viewLinkTable && $pSet->showViewLink( $this->field ) ) {

					$viewLinkLookup = $pSet->viewLinkLookup( $this->field );
					$viewLinkMethod = $pSet->viewLinkMethod( $this->field );
					$viewLinkPage = $pSet->viewLinkPage( $this->field );
					$linkFieldValue = $this->linkFieldValue;
					if( $linkFieldValue === NULL ) {
						//	display field == link field
						$linkFieldValue = $value;
					}


					$linkPSet = new ProjectSettings( $viewLinkTable, 'view' );
					//	make View page key field values
					if( $viewLinkLookup ) {
						//	data[field] holds display field value
						$keyFieldValues = array( $linkFieldValue );
					} else {
						$masterKeyFields = $linkPSet->getTableKeys();
						$masterKeys = $pSet->getMasterKeys( $viewLinkTable );
						
						$keyFieldValues = array();
						foreach( $masterKeyFields as $mk ) {
							$ki = array_search( $mk, $masterKeys['masterKeys'] );
							if( $ki !== false ) {
								$detKey = $masterKeys['detailsKeys'][ $ki ];
								if( $detKey == $this->field && $linkFieldValue !== null ) {
									//	data[field] holds display field value
									$keyFieldValues[] = $linkFieldValue;	
								} else {
									$keyFieldValues[] = $data[ $detKey ];
								}
							} else {
								$keyFieldValues[] = "";
							}
					...
					$params = array();
					foreach( $keyFieldValues as $i => $k ) {
						$params[] = "editid" . ( $i + 1 ) . '='. rawurlencode( $k );
					}
					if( $viewLinkPage ) {
						$params[] = 'page='.rawurlencode( $viewLinkPage );
					}
					$url = GetTableLink( $linkPSet->getShortTableName(), 'view', implode( '&', $params ) );
					$openAttr = "";
					if( $viewLinkMethod == vlmNewTab ) {
						$openAttr = 'target="_blank"';
					} else if( $viewLinkMethod == vlmPopup ) {
						$openAttr = 'data-viewlink-table="' . runner_htmlspecialchars( $viewLinkTable ) . '"';
						$openAttr .= ' data-viewlink-keys="' . runner_htmlspecialchars( runner_json_encode( $keyFieldValues ) ) . '"';
						if( $viewLinkPage ) {
							$openAttr .= ' data-viewlink-page="' . $viewLinkPage . '"';
						}
						//$openAttr .= ' onclick="event.preventDefault()"';

					}
					$processedText = '<a href="' . runner_htmlspecialchars( $url ) . '" ' . $openAttr . '>' . $processedText . '</a>';
				}

				if( $pSet->showCopyLink( $this->field ) && strlen( $value ) ) {
					global $hiddenTextSize;
					$hiddenText = substr( $value, 0, $hiddenTextSize );
					$processedText .= '&nbsp;<a data-copy-text="' . runner_htmlspecialchars($hiddenText) . '" class="btn btn-link btn-xs" '
						.'title="' . runner_htmlspecialchars( mlang_message('COPY') ). '">'
